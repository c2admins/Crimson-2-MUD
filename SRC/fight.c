...
void ft3200_perform_hots_dots(void)
{
	struct char_data *ch;
	...
	/* DAMAGE DONE BY DOTS */
	if (IS_AFFECTED(ch, AFF_DOTS)) {
		for (af = ch->affected; af; af = next_af) {
			next_af = af->next;
			if (af->type == SPELL_DOT_1) {
				dots = dice(8, GET_LEVEL(ch)/5);
				act("&rA cold chill saps away your life force.\r\n&E", TRUE, ch, 0, victim, TO_CHAR);
				act("&r$N shivers as they are surrounded with ice.&E", TRUE, ch, 0, victim, TO_NOTVICT);
				DAMAGE(ch, ch, dam, SPELL_DOT_1);
			}
			if (af->type == SPELL_DOT_2) {
				dots = dice(10, GET_LEVEL(ch)/5);
				act("&rYou suffer from black fire that envelops you.\r\n&E", TRUE, ch, 0, victim, TO_CHAR);
				act("&r$N is enveloped in black fire.\r\n&E", TRUE, ch, 0, victim, TO_NOTVICT);
				DAMAGE(ch, ch, dam, SPELL_DOT_2);
			}
		}
	}			/* END OF DOTS */
	...
	if (IS_AFFECTED(ch, AFF_DOTS)) {
		for (af = ch->affected; af; af = next_af) {
			next_af = af->next;
			if (af->type == SPELL_MENDING) {
				hots = dice(3, GET_LEVEL(ch) / 4) + ((GET_STR(ch) * GET_INT(ch)) / 2);
			act("&WMagical energy begins to heal your wounds.&E", TRUE, ch, 0, victim, TO_CHAR);
			act("&W$N's wounds seem to be healing rapidly.\r\n&E", TRUE, ch, 0, victim, TO_NOTVICT);
...
}               /* End of ft3200_perform_hots_dots*/