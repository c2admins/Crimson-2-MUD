              ...
              if (lv_mob_num == MOB_XANTH_ELEVATOR_REPAIRMAN) {
                  if (world[ch->in_room].number ==
					 XANTH_MOUSEVATOR_ENTR) {
                      ...
                      if (GET_GOLD(ch) > 1000) {
                          GET_GOLD(ch) = 1000;
                          xn4000_xanth_outside_mousevator
                                                (ch,CMD_PUSH,"lever"); 
                          xn4000_xanth_outside_mousevator
                                                (ch,CMD_PULL,"lever"); 
                          continue;
                      } /* END OF OVER DEFAULT GOLD */
                      else {
                          /* PUT HIM BACK IN ELEVATOR */
                          mo1500_do_move(ch, "", CMD_SOUTH);
                      }
                  } /* END OF WE ARE IN ELEVATOR ENTRY */
                  if (world[ch->in_room].number ==
					 XANTH_MOUSEVATOR_ROOM) {
                      /* HAVE WE BEEN GIVEN MONEY? */
                      if (GET_GOLD(ch) > 1000) {
                          GET_GOLD(ch) = 1000;
                          /* PUT HIM IN ELEVATOR ENTRY */
          ...
          if (lv_mob_num > 2689 && lv_mob_num < 2700) {
              if (!ch->master &&
                  !ch->followers) {
                   ...
                   ha3000_extract_char(ch, END_EXTRACT_BY_18);
                   continue;
              }

              /* IF DEMON ISN'T IN THE SAME ROOM, WITH ITS MASTER, */
              /* TELEPORT TO HIM/HER                               */
              if (ch->master) {
...
void mb1100_enfyrn_priest_changes_to_dragon(struct char_data *ch) {
   ...
   GVLOCATION(9510);
    ...
    /* REMOVE THE PRIEST */
    ha3000_extract_char(ch, END_EXTRACT_BY_19);
...
} /* END OF mb1100_enfyrn_priest_changes_to_dragon() */
...
void mb1125_enfyrn_dragon_changes_to_priest(struct char_data *ch) {
   ...
   GVLOCATION(9520);
    ...
    /* REMOVE THE DRAGON */
    ha3000_extract_char(ch, END_EXTRACT_BY_20);
...
} /* END OF mb1125_enfyrn_dragon_changes_to_priest() */
...
int mb1200_mob_artificial_int(struct char_data *ch, int lv_flag) {
   ...
   GVLOCATION(9530);
    ...
    if (ch->specials.fighting) {
        ...
        if (ch->skills[SPELL_SANCTUARY_MAJOR].learned > 0 &&
            !IS_AFFECTED(ch, AFF_SANCTUARY_MAJOR)) {
              rc = mb1300_mob_cast(ch, SPELL_SANCTUARY_MAJOR);
              return (rc);
        }
        rc = 0;
        if (!number(0,3))
...
int mb1300_mob_cast(struct char_data *ch, int lv_spell) {
   ...
   GVLOCATION(9535);
	...
	case SPELL_SANCTUARY_MAJOR : {
            say_spell(ch, SPELL_SANCTUARY_MAJOR);
            spell_sanctuary_major(GET_LEVEL(ch), ch, victim, 0);
	    break;
	}
...
struct char_data *mb1500_pick_victim(
   ...
   GVLOCATION(9545);
    ...
    if (IS_SET(spell_info[lv_spell].spell_flag, VIOLENT_SPELL) ||
   ...
   GVLOCATION(9555);
    ...
    for (obj=ch->carrying; obj; obj=next_obj) {
        ...
        if (number(1,2) == 1) {
            ...
            sprintf (buf,"%s entered: junk obj %s",
				GET_REAL_NAME(ch),
				GET_OBJ_NAME(obj));
	    do_wizinfo (buf,IMO_LEV5,ch);
